...
2 space-separated values on a single line.
The first value is a friend's name, and the second value is an 8-digit phone number.

After the n lines of phone book entries, there are an unknown number of lines of queries.
Each line (query) contains a name to look up, and you must continue reading lines
...
in the phone book; otherwise, print the full name and phoneNumber in the format name=phoneNumber.
...
for n in range(0,t):
    ...
    phone_book_entry = entry.lower().split(" ")
    if len(phone_book_entry) == 1:
        phone_book_entry.append("0")
    name = str(phone_book_entry[0])
    phone = str(phone_book_entry[1])
    phone = phone[0:8]
    while len(phone) < 8:
        phone = " " + phone
